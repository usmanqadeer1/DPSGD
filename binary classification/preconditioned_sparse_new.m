function [cost0] = preconditioned_sparse_new()
batch_size = 100;
...
Qo_left = zeros(size(W1, 1)); % kron(Q1_right'*Q1_right, Q1_left'*Q1_left) will be the preconditioner for W1
Qo_right = zeros(size(W1, 2));    
Q2_old = zeros(length(W2));
...
for iter = 1 : max_iter
    ...
    %%% added by me %%%%%
    Qn_left = ((1-gamma) * Q1_left + gamma * Qo_left)/ (1- gamma);
    Qn_right = ((1-gamma) * Q1_right + gamma * Qo_right)/ (1- gamma);
    Q2_new = ((1-gamma) * Q2 + gamma * Q2_old)/ (1- gamma);
    %%% added by me %%%%%
    W1 = W1 - 0.1*Qn_left'*Qn_left*grad_W1*Qn_right'*Qn_right;  % preconditioned SGD
    W2 = W2 - 0.1*(Q2_new'*Q2_new*grad_W2')';
